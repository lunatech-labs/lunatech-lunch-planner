...
object MenuPerDayPerPersonTable {
  ...
  def getAllUpcomingSchedulesByUser(userUuid: UUID)(implicit connection: DBConnection) : Future[Seq[MenuPerDayPerPerson]] = {
    implicit val getMenuPerDayPerPersonResult = GetResult(r => MenuPerDayPerPerson(UUID.fromString(r.<<), UUID.fromString(r.<<), UUID.fromString(r.<<), r.<<))

    val query = sql"""SELECT a."uuid", a."menuPerDayUuid", a."userUuid", a."isAttending"
                      FROM "MenuPerDayPerPerson" AS a JOIN "MenuPerDay" AS b ON a."menuPerDayUuid"=b.uuid
                      WHERE a."userUuid"='#$userUuid' AND b."date" >= current_date""".as[MenuPerDayPerPerson]
