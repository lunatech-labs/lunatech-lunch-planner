...
class ReportController @Inject()(userService: UserService,
  ...
  def filterAttendees: EssentialAction = adminAction.async { implicit request =>
    {
      ReportForm.reportForm.bindFromRequest
        .fold(
          _ => {
            Future.successful(
              Redirect(lunatech.lunchplanner.controllers.routes.ReportController
                .getReport()))
          },
          selectedReportDate => {
            val session = request.session + (month -> Integer.toString(
              selectedReportDate.month)) + (year -> Integer.toString(
  ...
  def export: EssentialAction = adminAction.async { implicit request =>
    ...
    for {
      ...
      val content =
        StreamConverters.fromInputStream(() => inputStream, ChunkSize)
      Result(
        header = ResponseHeader(
          Http.Status.OK,
          Map(
            Http.HeaderNames.CONTENT_DISPOSITION -> s"attachment; filename=${month} report.xls")),
        body =
