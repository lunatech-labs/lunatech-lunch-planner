      ...
      for {
        ...
        result <- MenuPerDayPerPersonForm.menuPerDayPerPersonForm.bindFromRequest
          .fold(
            formWithErrors =>
              for {
                menusPerDayPerPerson <- menuPerDayPerPersonService
                  .getAllMenuWithNamePerDayWithDishesPerPerson(getCurrentUser(
                    currentUser,
                    isAdmin = false,
                    request.email).uuid)
              } yield {
                BadRequest(
                  views.html.menuPerDayPerPerson(
                    getCurrentUser(currentUser,
                                   isAdmin = userService.isAdminUser(
                                     currentUser.get.emailAddress),
                                   request.email),
            ...
            formData => {
              updateData(getCurrentUser(currentUser,
                                        isAdmin = false,
                                        request.email).uuid,
                         formData).map { _ =>
                Redirect(
