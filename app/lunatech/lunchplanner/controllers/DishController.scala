...
class DishController @Inject() (
  ...
  def createNewDish: Action[AnyContent] = adminAction.async { implicit request =>
      DishForm
      .dishForm
      .bindFromRequest
      .fold(
  ...
  def getDishDetails(uuid: UUID): Action[AnyContent] = adminAction.async { implicit request =>
      for{
        currentUser <- userService.getByEmailAddress(request.email)
  ...
  def saveDishDetails(uuid: UUID): Action[AnyContent] = adminAction.async { implicit request =>
      DishForm
      .dishForm
      .bindFromRequest
      .fold(
  ...
  def deleteDishes(): Action[AnyContent] = adminAction.async { implicit request =>
    ListDishesForm
    .listDishesForm
    .bindFromRequest
    .fold(
  ...
  def deleteDish(uuid: UUID): Action[AnyContent] = adminAction.async { implicit request =>
