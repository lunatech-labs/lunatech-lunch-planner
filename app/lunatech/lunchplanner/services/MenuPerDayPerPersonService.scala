...
class MenuPerDayPerPersonService @Inject() (
  ...
  def getAll(userUuid: UUID): Future[Seq[MenuWithNamePerDayPerPerson]] = {
    def getSchedule(
        menuWithNamePerDay: MenuWithNamePerDay
  ...
  def getAllMenuWithNamePerDayWithDishesPerPerson(
      userUuid: UUID
  ): Future[Seq[MenuWithNameWithDishesPerPerson]] = {
    def retrieveInfoForMenu(
        menuWithNamePerDay: MenuWithNamePerDay
  ...
  def getByUserUuidAndMenuPerDayUuid(
    ...
    MenuPerDayPerPersonTable.getByUserUuidAndMenuPerDayUuid(
      userUuid,
      menuPerDayUuid
    )
  ...
  def getAllMenuWithNamePerDay: Future[Seq[MenuWithNamePerDay]] = {
    ...
    ): Future[Option[MenuWithNamePerDay]] =
      for {
  ...
  def getListOfPeopleByMenuPerDay(
      menuPerDayUuid: UUID
  ): Future[Seq[MenuPerDayAttendant]] =
    for {
  ...
  def getListOfPeopleByMenuPerDayByLocationAndDateForReport(
    ...
    MenuPerDayPerPersonTable
      ...
      .map(
        _.map(user =>
          MenuPerDayReportByDateAndLocation(
            menuPerDay.date,
            menuPerDay.location,
            user._1.name
          )
  ...
  private def isAttending(
      menuPerDayPerPerson: Option[MenuPerDayPerPerson]
  ): Option[Boolean] =
