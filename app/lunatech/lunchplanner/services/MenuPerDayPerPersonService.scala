...
class MenuPerDayPerPersonService  @Inject() (
  ...
  def getAllMenuWithNamePerDayWithDishesPerPerson(userUuid: UUID): Future[Seq[MenuWithNameWithDishesPerPerson]]  = {
    getAllMenuWithNamePerDay.flatMap {
      Future.traverse(_) { menuWithNamePerDay =>
        MenuDishTable.getByMenuUuid(menuWithNamePerDay.menuUuid).flatMap(
          Future.traverse(_)(dish =>
            dishService.getByUuid(dish.dishUuid)).map(_.flatten)).flatMap { dishes =>
            getByUserUuidAndMenuPerDayUuid(userUuid, menuWithNamePerDay.uuid).map { menuPerDayPerPerson =>
  ...
  def getListOfPeopleByMenuPerDay(menuPerDayUuid: UUID, isAttending: Boolean): Future[Seq[MenuPerDayAttendant]] = {
    MenuPerDayPerPersonTable.getAttendeesByMenuPerDayUuid(menuPerDayUuid, isAttending)
  ...
  private def isAttending(menuPerDayPerPerson: Option[MenuPerDayPerPerson]) : Option[Boolean] = {
    menuPerDayPerPerson.map(m => m.isAttending)
  }
