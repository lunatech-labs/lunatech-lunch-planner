...
class MenuPerDayPerPersonService @Inject()(
  ...
  def getAllMenuWithNamePerDayWithDishesPerPerson(
    ...
    getAllMenuWithNamePerDay.flatMap { allMenuWithNamePerDay =>
      Future.traverse(allMenuWithNamePerDay) { menuWithNamePerDay =>
        MenuDishTable
          .getByMenuUuid(menuWithNamePerDay.menuUuid)
          .flatMap(
            menuDishes =>
              Future
                .traverse(menuDishes)(dish =>
                  dishService.getByUuid(dish.dishUuid))
                .map(a => a.flatten))
          .flatMap { dishes =>
            getByUserUuidAndMenuPerDayUuid(userUuid, menuWithNamePerDay.uuid)
              .map { menuPerDayPerPerson =>
  ...
  def getAllMenuWithNamePerDayFilterDateRange(
    ...
    menuPerDayService
      .getAllOrderedByDateFilterDateRange(dateStart, dateEnd)
      .flatMap {
        Future.traverse(_) { menuPerDay =>
          menuService.getByUuid(menuPerDay.menuUuid).flatMap {
            case Some(menuData) =>
  ...
  def getListOfPeopleByMenuPerDay(
    ...
    MenuPerDayPerPersonTable
      .getAttendeesByMenuPerDayUuid(menuPerDayUuid)
      .map(_.map {
