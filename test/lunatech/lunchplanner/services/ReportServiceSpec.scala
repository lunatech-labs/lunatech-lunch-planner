...
class ReportServiceSpec extends BehaviorTestingConfig with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    ...
    for {
      ...
      menuperday1 <- MenuPerDayTable.add(MenuPerDay(menuUuid = menu1.uuid, date = new java.sql.Date(1515106800000L), location = "Rotterdam"))
      menuperday2 <- MenuPerDayTable.add(MenuPerDay(menuUuid = menu2.uuid, date = new java.sql.Date(1515106800000L), location = "Amsterdam"))
      ...
      menuperday3 <- MenuPerDayTable.add(MenuPerDay(menuUuid = menu1.uuid, date = new java.sql.Date(1518217200000L), location = "Rotterdam"))
      menuperday4 <- MenuPerDayTable.add(MenuPerDay(menuUuid = menu2.uuid, date = new java.sql.Date(1518217200000L), location = "Amsterdam"))
  ...
  "Report service" should {
    "produce list of attendees per location by date for January 2018" in {
      ...
      result.usersPerDateAndLocation mustBe Vector(
        ((new java.sql.Date(1515106800000L), "Amsterdam"), Vector("user 2")),
        ((new java.sql.Date(1515106800000L), "Rotterdam"), Vector("user 1", "user 3")))
    }
    ...
    "produce list of attendees per location by date for February 2018" in {
      ...
      result.usersPerDateAndLocation mustBe Vector(
        ((new java.sql.Date(1518217200000L), "Amsterdam"), Vector("user 1", "user 2", "user 4")))
    }
