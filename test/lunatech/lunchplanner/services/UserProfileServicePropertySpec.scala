...
object UserProfileServicePropertySpec
  ...
  property("insert new user profile") = forAll {
  ...
  property("update an existent user profile") = forAll {
    (user: User, userProfile1: UserProfile, userProfile2: UserProfile) =>
      createTestSchema()

      addUserAndProfileToDB(user, userProfile1)

      val result = Await.result(
        userProfileService.update(userProfile2.copy(userUuid = user.uuid)),
        defaultTimeout
      )
      val updatedProfile = Await
        .result(UserProfileTable.getByUserUUID(user.uuid), defaultTimeout)
        .get
  ...
  property("return user profile given user uuid") = forAll {
    (user: User, userProfile: UserProfile) =>
      createTestSchema()
      ...
      val result = Await
        .result(
          userProfileService.getUserProfileByUserUuid(user.uuid),
          defaultTimeout
        )
        .get
