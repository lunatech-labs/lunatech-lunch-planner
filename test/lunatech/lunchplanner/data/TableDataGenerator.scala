...
import lunatech.lunchplanner.models.{ Dish, Menu, MenuDish, MenuPerDay, MenuPerDayPerPerson, User, UserProfile }
...
object TableDataGenerator {
  ...
  implicit val arbitraryUserProfile: Arbitrary[UserProfile] = Arbitrary[UserProfile] {
    for {
      uuid <- Gen.uuid
  ...
  // replaceAll("\u0000", "") to avoid the error: 'invalid byte sequence for encoding "UTF8": 0x00'
  implicit val arbitraryMenuPerDay: Arbitrary[MenuPerDay] = Arbitrary[MenuPerDay] {
    for {
      uuid <- Gen.uuid
      menuUuid <- Gen.uuid
  ...
  implicit val arbitraryMenuPerDayPerPerson: Arbitrary[MenuPerDayPerPerson] = Arbitrary[MenuPerDayPerPerson] {
    for {
      uuid <- Gen.uuid
